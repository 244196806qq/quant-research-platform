...
def compute_exposure(sizing_mode: str, clean_sig: pd.Series, returns_series: pd.Series, params: dict, sizing_lookback: int, vol_target: float, max_leverage: float, min_exposure: float, max_exposure: float) -> (pd.Series, pd.Series):
    exposures = []
    # sizing model
    if sizing_mode == "fixed":
        ...
        exposures.append(exposure)
        rv = realized_volatility(returns_series, lookback=sizing_lookback)
    elif sizing_mode == "drawdown_based":
        raise NotImplementedError("Drawdown sizing needs an equity curve, not a signal.")
        # exposure = drawdown_based_size(clean_sig, lookback=sizing_lookback)
        # exposures.append(exposure)
        # rv = realized_volatility(returns_series, lookback=sizing_lookback)
    else:
...
def simulate_strategy(dfs, holding, symbols, strategy, initial_money, deposit, indicators=None, params=None, diagnostics=None, allow_leverage: bool = False, fee: float = 0.0, slippage: float = 0.0) -> (pd.Series, pd.Series):
    """
    ...
    for i in range(len(dfs)):
        ...
        exposure, rv = compute_exposure(sizing_mode, clean_sig, returns_series, params, sizing_lookback, vol_target, max_leverage, min_exposure, max_exposure)
    ...
    return df_strategy["Market_Equity"], total_abs_exposure
