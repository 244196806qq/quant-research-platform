...
def build_metrics(df_strategy: pd.DataFrame, dfs: dict, active: list, exposures: pd.DataFrame, final_money: float) -> pd.DataFrame:
    rows = []
        ...
        rows.append(
            {
                ...
                "Sharpe Ratio": round(calculate_sharpe_ratio(returns), 2),
                "Number of Trades": 0,
                "Average Exposure": round((exposures[strategy]).mean(), 2),
            }
        )
...
def main():
    ...
    if run:
        ...
        strategies_to_run = ["Buy and Hold All"] + strategies  # always include buy and hold for comparison
        
        st.write("Selected strategies:", strategies)
        if not active:
            ...
            try:
                with st.spinner("Loading data and running backtest..."):
                    ...
                    buy_hold = pd.concat([buy_hold, df_strategy["Buy and Hold All"].to_frame()], axis=1)
                    #buy_hold = df_strategy["Buy and Hold"].to_frame()
            ...
            with tabs[3]:
                st.header("Metrics")
                metrics_df = build_metrics(safe_df_strategy, dfs, active, exposures, final_money)
