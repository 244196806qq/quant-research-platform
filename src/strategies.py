...
def buy_hold_strategy(df: pd.DataFrame, params: dict, holdings: dict, indicators: dict = None):
...
def ma_strategy(df: pd.DataFrame, params: dict, holdings: dict, indicators: dict = None):
    ma_df = calculate_indicator(df, "MA", short=params.get("fast_ma", 10), mid=params.get("mid_ma", 50), long=params.get("slow_ma", 200))
    signal = _make_series(df.index, 1.0)
...
def zscore_strategy(df: pd.DataFrame, params: dict, holdings: dict, indicators: dict = None):
    s = calculate_indicator(df, "Z-Score", window = params.get("window", 20)) #, window=params.get("zscore_window", 20))
    signal = _make_series(df.index, 1.0)
...
def momentum_strategy(df: pd.DataFrame, params: dict, holdings: dict, indicators: dict = None):
    m = calculate_indicator(df, "Momentum", window=params.get("window", 50))
...
def rsi_strategy(df: pd.DataFrame, params: dict, holdings: dict, indicators: dict = None):
    r = calculate_indicator(df, "RSI", window=params.get("window", 14))
    signal = _make_series(df.index, 1.0)
...
def rolling_std_strategy(df: pd.DataFrame, params: dict, holdings: dict, indicators: dict = None):
    vol = calculate_indicator(df, "Rolling Std", window=params.get("window", 20))
    signal = _make_series(df.index, 1.0)
...
def atr_strategy(df: pd.DataFrame, params: dict, holdings: dict, indicators: dict = None):
    atr_s = calculate_indicator(df, "Avg True Range", window=params.get("window", 14))
    signal = _make_series(df.index, 1.0)
...
def ewma_strategy(df: pd.DataFrame, params: dict, holdings: dict, indicators: dict = None):
    e = calculate_indicator(df, "EWMA", span=params.get("span", 20))
...
def annual_std_strategy(df: pd.DataFrame, params: dict, holdings: dict, indicators: dict = None):
    a = calculate_indicator(df, "Annual Std", window=params.get("window", 20))
    signal = _make_series(df.index, 1.0)
...
STRATEGY_REGISTRY = {
    "Buy and Hold All": buy_hold_strategy,  # This would be defined similarly to the others, but just returns a signal of 1.0
    "MA": ma_strategy,
...
STRATEGY_METADATA = {
    ...
    "Z-Score": {
        ...
        "parameters": {"window": 20, "zscore_low": -2.0, "zscore_high": 2.0},
    },
    "RSI": {
        "name": "RSI",
        "description": "Mean-reversion using RSI thresholds.",
        "category": "Mean Reversion",
        "parameters": {"window":14, "rsi_lower": 30, "rsi_upper": 70, "rsi_overshoot": 2.0, "rsi_cooldown": 0.2},
    },
    "Rolling Std": {
        "name": "Rolling Std",
        "description": "Volatility-based exposure scaling.",
        "category": "Volatility",
        "parameters": {"window": 20, "vol_low": 0.01, "vol_mid": 0.02, "vol_high": 0.03},
    },
    "Avg True Range": {
        "name": "Avg True Range",
